...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    str_imgs_train = [os.path.join(data_file_base_dir, item['image'][0] if isinstance(item['image'], list) else item['image'])for item in tr_files if item['image']]
    ...
    # Add reference label to tr_files and val_files
    for item, str_ref_t in zip(tr_files, str_ref_seg_tr):
    ...
    if torch.cuda.device_count() > 1:
        tr_files = partition_dataset(data=tr_files, shuffle=False, num_partitions=world_size, even_divisible=True)[
            dist.get_rank()
        ]

    # Get just the image paths as list: TxFLAIR comparison
    logger.debug(f"Train_files: {len(tr_files)}")
        ...
        if (epoch + 1) % val_interval == 0 or (epoch + 1) == num_epochs:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    logger.debug(f"{_index + 1} / {len(val_loader)} 'dice_value': {value}, 'reference value': {ref_value}") 
                    ...
                    metric_sum += value.sum().item(); ref_metric_sum += ref_value.sum().item()
                    metric_vals = value.cpu().numpy(); ref_metric_vals = ref_value.cpu().numpy()
