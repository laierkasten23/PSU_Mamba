        ...
        if (epoch + 1) % val_interval == 0 or (epoch + 1) == num_epochs:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    value = compute_dice(y_pred=val_outputs, y=val_labels, include_background=False).to(device)
                ...
                if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
                    ...
                    logger.debug(
                        "Current epoch: {} current mean hausdorff: {:.4f}, current reference mean hausdorff: {:.4f},  best mean hausdorff: {:.4f} at epoch {}".format(
                            epoch + 1, avg_hd_metric, avg_hd_metric_ref, avg_hd_metric, avg_hd_metric_ref, best_hd_metric, best_hd_metric_epoch
                        )
                    )
