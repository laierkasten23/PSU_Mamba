...
class CosineAnnealingLRWithWarmup(_LRScheduler):
    def __init__(
        self, optimizer, T_max, eta_min=0, last_epoch=-1, verbose=False, warmup_epochs=2, warmup_multiplier=0.1
    ):
        self.T_max = T_max
        ...
        if self.warmup_epochs > 0 and self.last_epoch < self.warmup_epochs:
            lr = [
                ...
                * (
                    ...
                    + (1 - self.warmup_multiplier) * (max(0, self.last_epoch) / float(self.warmup_epochs))
                )
            ...
            lr = [
                self.eta_min + (base_lr - self.eta_min) * (1 + math.cos(math.pi * fraction)) / 2
                for base_lr in self.base_lrs
            ]
