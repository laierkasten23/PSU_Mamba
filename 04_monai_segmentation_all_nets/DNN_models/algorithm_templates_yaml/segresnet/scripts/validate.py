...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    validate_transforms = parser.get_parsed_content("transforms_validate")
    ...
    post_transforms = [
        transforms.Invertd(keys="pred", transform=validate_transforms, nearest_interp=False, to_tensor=True)
    ]
    ...
    if save_mask:
        post_transforms += [
            transforms.SaveImaged(keys="pred", output_dir=output_path, output_postfix="seg", resample=False)
        ]
    ...
    with torch.no_grad():
        ...
        for d in val_loader:
            ...
            print(val_images.shape, val_labels.shape, val_outputs.shape, patch_size_valid)
