...
class BinaryMaskd(MapTransform):
    def __init__(self, keys: KeysCollection, allow_missing_keys: bool = False):
        ...
        converts a given input into a binary mask. It sets all non-zero values in the input to 1.
...
class GetCentroidsd(MapTransform):
    def __init__(self, keys: KeysCollection, centroids_key: str = "centroids", allow_missing_keys: bool = False):
        """
        Computes the centroids of all segments in the input data. It skips the background and computes the center
        of mass for each segment.
...
class VertHeatMap(MapTransform):
    """
    Transformation that creates a heatmap for each segmentation class in the input data. 
    It computes the center of mass for each class and applies a Gaussian smoothing operation to it.
    """
